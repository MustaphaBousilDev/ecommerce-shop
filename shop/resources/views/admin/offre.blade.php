                    ...
                    <div class="body__products-dashboard">
                         <form class='mt-3'>
                              ...
                              bg-color-red-button w-[95%] md:w-[60%] overflow-hidden rounded-md'>
                                   <input type='search' id='search-offre' class='py-3  px-8 w-[100%] h-[100%]' name='search' placeholder="Search" />
...
<div class='w-[500px] h-[60vh]  bg-primary-50 rounded-lg z-[10000] fixed top-[50%] translate-y-[-50%] left-[50%] translate-x-[-50%] active-showing model_form '>
    ...
    <form action='' class='w-full form-add' id='form-add' enctype="multipart/form-data">
        @csrf
        <h3 class='text-center heading-form'>Add Offres</h3>
...
<div class='w-[500px] h-[60vh]  bg-primary-50 rounded-lg z-[10000] fixed top-[50%] translate-y-[-50%] left-[50%] translate-x-[-50%] active-showing model_form_edit '>
    ...
    <form action='' class='w-full form-edit' id='form-edit' enctype="multipart/form-data">
        @csrf
        <h3 class='text-center heading-form'>Edit Offres</h3>
        
            <div class="w-[80%] mx-auto">
                <input type='hidden'  name='id' id='id_offre'>
        ...
        <!--model insert categories to database-->
         ...
         <table class="w-full text-sm text-left text-blue-100 dark:text-blue-100">
              <thead class="text-xs text-white uppercase bg-blue-600 dark:text-white">
                   <tr>
                        <th scope="col" class="px-6 py-3">Offres</th>
              ...
              <tbody class='body__table'>
                    @foreach ($offres as $offre)
                        <tr class="bg-blue-500 border-b border-b-color-gray-background-light border-blue-400">
                            <th scope="row" class="px-6 py-4 flex items-center gap-2 font-medium text-blue-50  dark:text-blue-100">
                                <span><img src="{{ asset('offres/'.$offre->image) }}" alt="products" class='w-14 h-14 rounded-full' />
                                </span>
                                <span>{{$offre->name}}</span>
                            </th>
                            <td scope="" class="px-6 py-4   dark:text-blue-100">
                                <span>{{$offre->percent}}%</span>
                            </td>
                            <td scope="" class="px-6 py-4   dark:text-blue-100">
                                <span>{{$offre->sub_category_id}}</span>
                            </td>
                            <td class="px-6 py-4">
                                <div class='flex gap-1 items-center'>
                                    <span onclick='edit_offre()' class='button_edit' data-id='{{$offre->id}}'>
                                    ...
                                    <span class='button_delete' data-id='{{$offre->id}}'><i class='bx bxs-trash text-2xl rounded-full flex items-center justify-center cursor-pointer w-11 h-11 bg-[#fee2e2] text-[#f87171]' ></i></span>
    ...
    $(document).ready(function(){
        ...
        $.ajax({
            ...
            success: function (response){
                console.log('success')
                if(response.status=="success inserted offres"){
                    $('#form-add')[0].reset()
                    
                    //console.log(response)
                    $('.body__table').empty()
                    $.each(response.offres,function(key,value){
                        console.log(value)
                        //empty table 
                        $('.body__table').append(`
                        <tr class="bg-blue-500 border-b border-b-color-gray-background-light border-blue-400">
                            <th scope="row" class="px-6 py-4 flex items-center gap-2 font-medium text-blue-50 whitespace-nowrap dark:text-blue-100">
                                <span><img src="{{ asset('offres/${value.image}') }}" alt="products" class='w-14 h-14 rounded-full' />
                                </span>
                                <span>${value.name}</span>
                            </th>
                            <td  class="px-6 py-4  dark:text-blue-100">
                                <span>${value.percent}%</span>
                            </td>
                            <td  class="px-6 py-4  dark:text-blue-100">
                                <span>${value.sub_category_id}</span>
                            </td>
                            <td class="px-6 py-4">
                                <div class='flex gap-1 items-center'>
                                    <span onclick='edit_offre()' class='button_edit' data-id='${value.id}'><i class='bx bxs-edit text-2xl rounded-full flex items-center justify-center cursor-pointer w-11 h-11 bg-[#dcfce7] text-[#4ade80]' ></i></span>
                                    <span data-id='${value.id}'><i class='bx bxs-trash text-2xl rounded-full flex items-center justify-center cursor-pointer w-11 h-11 bg-[#fee2e2] text-[#f87171]' ></i></span>
                                    <span><i class='bx bxs-detail text-2xl rounded-full flex items-center justify-center cursor-pointer w-11 h-11 bg-[#cffafe] text-[#22d3ee]' ></i></span>
                                </div>
                            </td>
                        </tr>
        ...
        $(document).on('click','.button_edit',function (e){
            ...
            $.ajax({
                ...
                success: function (response){
                    console.log(response)
                    $('#name_edit').val(response.offres.name)
                    $('#percent_edit').val(response.offres.percent)
                    $('#subcategory_id_edit').val(response.offres.sub_category_id)
                    $('#id_offre').val(response.offres.id)
                    $('#old_image').val(response.offres.image)

                    
                    //get element has class image-div-edit and set attribute src to image
                    //$('.image-div-edit').style.backgroundImage=`url({{ asset('offres/${response.offres.image}') }})`
                },
                error: function (error){
                    console.log("error")
                    $.each(error.responseJSON.errors,function(key,value) {
        ...
        $(document).on('click','.save_button_edit',function(e){
            ...
            $.ajax({
                ...
                success: function (response){
                    console.log('success')
                    if(response.status=="success updated offre"){
                        $('#form-edit')[0].reset()
                        $('.body__table').empty()
                        $.each(response.offres,function(key,value){
                            console.log(value)
        ...
        $(document).on('click','.button_delete',function(e){
            e.preventDefault()
            let id = $(this).data('id')
            if(confirm('are you sure to delete this offre?')){
                $.ajax({
                    url:"{{route('offre-delete')}}",
                    type: 'POST',
                    data: {
                        id:id,
                    ...
                    success: function (response){
                        if(response.status=="success deleted offre"){
                            console.log('success')
                            $('.body__table').empty()
                            $.each(response.offres,function(key,value){
                                console.log(value)
        ...
        //search
        $(document).on('keyup','#search-offre',function(e){
            e.preventDefault()
            let search = $(this).val()
            console.log(search)
            $.ajax({
                url:"{{route('offre-search')}}",
                type: 'POST',
                data: {
                    search:search,
                    _token:$('meta[name="csrf-token"]').attr('content')
                ...
                success: function (response){
                    if(response.status=="success"){
                        ...
                        var load = true
                            ...
                            //filter table of offres by search 
                            $('.body__table').empty()
                            $.each(response.offres,function(key,value){
                                console.log(value)
                                //empty table 
