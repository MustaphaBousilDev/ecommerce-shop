                    ...
                    <div class="body__products-dashboard">
                         <form class='mt-3'>
                              ...
                              bg-color-red-button w-[95%] md:w-[60%] overflow-hidden rounded-md'>
                                   <input type='search' id='search-brands' class='py-3  px-8 w-[100%] h-[100%]' name='search' placeholder="Search" />
...
<div class='w-[500px] h-[60vh]  bg-primary-50 rounded-lg z-[10000] fixed top-[50%] translate-y-[-50%] left-[50%] translate-x-[-50%] active-showing model_form '>
    ...
    <form action='' class='w-full form-add' id='form-add' enctype="multipart/form-data">
        @csrf
        <h3 class='text-center heading-form'>Add Brands</h3>
                    ...
                    @foreach ($brands as $brand)
                        <tr class="bg-blue-500 border-b border-b-color-gray-background-light border-blue-400">
                            <th scope="row" class="px-6 py-4 flex items-center gap-2 font-medium text-blue-50 whitespace-nowrap dark:text-blue-100">
                                <span><img src="{{ asset('brands/'.$brand->image) }}" alt="products" class='w-14 h-14 rounded-full' />
                                </span>
                            ...
                            <td class="px-6 py-4">
                                <div class='flex gap-1 items-center'>
                                    <span onclick='edit_category()' class='button_edit' data-id='{{$brand->id}}'>
                                    ...
                                    <span class='button_delete' data-id='{{$brand->id}}'><i class='bx bxs-trash text-2xl rounded-full flex items-center justify-center cursor-pointer w-11 h-11 bg-[#fee2e2] text-[#f87171]' ></i></span>
    ...
    $(document).ready(function(){
        ...
        $.ajax({
            ...
            success: function (response){
                console.log('success')
                if(response.status=="success inserted brand"){
                    $('#form-add')[0].reset()
                    ...
                    $.each(response.brands,function(key,value){
                        ...
                        <tr class="bg-blue-500 border-b border-b-color-gray-background-light border-blue-400">
                            <th scope="row" class="px-6 py-4 flex items-center gap-2 font-medium text-blue-50 whitespace-nowrap dark:text-blue-100">
                                <span><img src="{{ asset('brands/${value.image}') }}" alt="products" class='w-14 h-14 rounded-full' />
                                </span>
        ...
        $(document).on('click','.button_edit',function (e){
            ...
            $.ajax({
                ...
                success: function (response){
                    ...
                    $('#description_edit').val(response.brands.description)
                    $('#old_img').val(response.brands.image)
                    $('#id_brand').val(response.brands.id)
        ...
        $(document).on('click','.save_button_edit',function(e){
            ...
            $.ajax({
                ...
                success: function (response){
                    console.log('success')
                    if(response.status=="success updated brands"){
                        $('#form-edit')[0].reset()
                        $('.body__table').empty()
                        $.each(response.brands,function(key,value){
                            console.log(value)
        ...
        $(document).on('click','.button_delete',function(e){
            ...
            if(confirm('are you sure to delete this brands?')){
                $.ajax({
                    ...
                    success: function (response){
                        if(response.status=="success delete brands"){
                            console.log('success')
                            $('.body__table').empty()
                            $.each(response.brands,function(key,value){
                                console.log(value)
        ...
        //search
        $(document).on('keyup','#search-brands',function(e){
            e.preventDefault()
            let search = $(this).val()
            console.log(search)
            $.ajax({
                url:"{{route('brandss-search')}}",
                type: 'POST',
                data: {
                    search:search,
                    _token:$('meta[name="csrf-token"]').attr('content')
