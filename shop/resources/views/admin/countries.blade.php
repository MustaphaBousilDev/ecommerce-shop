                    ...
                    <div class="body__products-dashboard">
                         <form class='mt-3'>
                              ...
                              bg-color-red-button w-[95%] md:w-[60%] overflow-hidden rounded-md'>
                                   <input type='search' id='search-country' class='py-3  px-8 w-[100%] h-[100%]' name='search' placeholder="Search" />
...
<div class='w-[500px] p-4  bg-primary-50 rounded-lg z-[10000] fixed top-[50%] translate-y-[-50%] left-[50%] translate-x-[-50%] active-showing model_form '>
    ...
    <form action='' class='w-full form-add' id='form-add' enctype="multipart/form-data">
        @csrf
        <h3 class='text-center heading-form'>Add Country</h3>
...
<div class='w-[500px] p-4  bg-primary-50 rounded-lg z-[10000] fixed top-[50%] translate-y-[-50%] left-[50%] translate-x-[-50%] active-showing model_form_edit '>
    ...
    <form action='' class='w-full form-edit' id='form-edit' enctype="multipart/form-data">
        @csrf
        <h3 class='text-center heading-form'>Edit Country</h3>
                    ...
                    @foreach ($countries as $country)
                        <tr class="bg-blue-500 border-b border-b-color-gray-background-light border-blue-400">
                            <th scope="row" class="px-6 py-4 flex items-center gap-2 font-medium text-blue-50 whitespace-nowrap dark:text-blue-100">
                                <span><img src="{{ asset('countries/'.$country->image) }}" alt="products" class='w-14 h-14 rounded-full' />
                                </span>
                            ...
                            <td class="px-6 py-4">
                                <div class='flex gap-1 items-center'>
                                    <span onclick='edit_category()' class='button_edit' data-id='{{$country->id}}'>
                                    ...
                                    <span class='button_delete' data-id='{{$country->id}}'><i class='bx bxs-trash text-2xl rounded-full flex items-center justify-center cursor-pointer w-11 h-11 bg-[#fee2e2] text-[#f87171]' ></i></span>
    ...
    $(document).ready(function(){
        ...
        $.ajax({
            ...
            success: function (response){
                console.log('success')
                if(response.status=="success inserted country"){
                    $('#form-add')[0].reset()
                    ...
                    $.each(response.country,function(key,value){
                        ...
                        <tr class="bg-blue-500 border-b border-b-color-gray-background-light border-blue-400">
                            <th scope="row" class="px-6 py-4 flex items-center gap-2 font-medium text-blue-50 whitespace-nowrap dark:text-blue-100">
                                <span><img src="{{ asset('countries/${value.image}') }}" alt="products" class='w-14 h-14 rounded-full' />
                                </span>
        ...
        $(document).on('click','.button_edit',function (e){
            ...
            $.ajax({
                ...
                success: function (response){
                    console.log(response)
                    $('#name_edit').val(response.country.name)
                    $('#old_img').val(response.country.image)
        ...
        $(document).on('click','.save_button_edit',function(e){
            ...
            $.ajax({
                ...
                success: function (response){
                    console.log('success')
                    if(response.status=="success updated country"){
                        $('#form-edit')[0].reset()
                        $('.body__table').empty()
                        $.each(response.country,function(key,value){
                            console.log(value)
        ...
        $(document).on('click','.button_delete',function(e){
            ...
            if(confirm('are you sure to delete this brands?')){
                $.ajax({
                    ...
                    success: function (response){
                        if(response.status=="success delete country"){
                            console.log('success')
                            $('.body__table').empty()
                            $.each(response.country,function(key,value){
                                console.log(value)
        ...
        //search
        $(document).on('keyup','#search-country',function(e){
            e.preventDefault()
            let search = $(this).val()
            console.log(search)
            $.ajax({
