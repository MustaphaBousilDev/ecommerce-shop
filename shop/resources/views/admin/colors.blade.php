...
<div class='w-[500px] h-[60vh]  bg-primary-50 rounded-lg z-[10000] fixed top-[50%] translate-y-[-50%] left-[50%] translate-x-[-50%] active-showing model_form '>
    ...
    <form action='' class='w-full form-add' id='form-add' enctype="multipart/form-data">
        @csrf
        <h3 class='text-center heading-form'>Add color</h3>
            ...
            <div class="w-[80%] mx-auto">
               <input type="text" name="code" placeholder='code' id="code" class="w-full h-[40px] border border-color-gray-background-light rounded-md px-3 mt-2 outline-none focus:border-color-red-button"/>
           </div>
...
<div class='w-[500px] h-[60vh]  bg-primary-50 rounded-lg z-[10000] fixed top-[50%] translate-y-[-50%] left-[50%] translate-x-[-50%] active-showing model_form_edit '>
    <form action='' class='w-full form-edit' id='form-edit' enctype="multipart/form-data">
        @csrf
        <h3 class='text-center heading-form'>Edit colore</h3>
            <div class="w-[80%] mx-auto">
                <input type='hidden'  name='id' id='id_color'>
            ...
            <div class="w-[80%] mx-auto">
               <input type="text" name="code_edit" placeholder='code_edit' id="code_edit" class="w-full h-[40px] border border-color-gray-background-light rounded-md px-3 mt-2 outline-none focus:border-color-red-button"/>
           </div>
                        ...
                        <tr class="bg-blue-500 border-b border-b-color-gray-background-light border-blue-400">
                            ...
                            <td class="px-6 py-4">
                                <div class='flex gap-1 items-center'>
                                    <span onclick='edit_category()' class='button_edit' data-id='{{$color->id}}'>
                                    ...
                                    <span class='button_delete' data-id='{{$color->id}}'><i class='bx bxs-trash text-2xl rounded-full flex items-center justify-center cursor-pointer w-11 h-11 bg-[#fee2e2] text-[#f87171]' ></i></span>
    ...
    $(document).ready(function(){
        ...
        $.ajax({
            ...
            success: function (response){
                console.log('success')
                if(response.status=="success inserted color"){
                    $('#form-add')[0].reset()
        ...
        $(document).on('click','.button_edit',function (e){
            ...
            $.ajax({
                ...
                success: function (response){
                    ...
                    $('#code_edit').val(response.color.code)
                    $('#id_color').val(response.color.id)
                    //get element has class image-div-edit and set attribute src to image
                    //$('.image-div-edit').style.backgroundImage=`url({{ asset('brands/${response.brands.image}') }})`
                },
                error: function (error){
                    console.log("error")
                    $.each(error.responseJSON.errors,function(key,value) {
        ...
        $(document).on('click','.save_button_edit',function(e){
            ...
            $.ajax({
                ...
                success: function (response){
                    console.log('success')
                    if(response.status=="success updated color"){
                        $('#form-edit')[0].reset()
                        $('.body__table').empty()
                        $.each(response.color,function(key,value){
                            console.log(value)
        ...
        $(document).on('click','.button_delete',function(e){
            ...
            if(confirm('are you sure to delete this size?')){
                $.ajax({
                    ...
                    success: function (response){
                        if(response.status=="success delete color"){
                            console.log('success')
                            $('.body__table').empty()
                            $.each(response.color,function(key,value){
                                console.log(value)
        ...
        //search
        $(document).on('keyup','#search-color',function(e){
            e.preventDefault()
            let search = $(this).val()
            console.log(search)
            $.ajax({
                ...
                success: function (response){
                    if(response.status=="success"){
                        ...
                        var load = true
                            ...
                            //filter table of brands by search 
                            $('.body__table').empty()
                            $.each(response.color,function(key,value){
                                console.log(value)
                                //empty table 
